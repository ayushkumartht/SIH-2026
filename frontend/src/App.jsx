...
function DoctorDashboard({ session, onLogout }) {
  ...
  async function load() {
    try {
      setData(await request("/api/portal/doctor/dashboard", session.token));
    } catch (e) {
      setMessage(e.message);
    }
  }
  ...
  async function save(event) {
    ...
    try {
      await request(
        `/api/doctors/consultations/${selected._id}`,
        session.token,
        {
          method: "PATCH",
          body: JSON.stringify({
  ...
  return (
    <Shell>
      <DashboardHeader
        title={`Good day, ${data.doctor.name}`}
        role="Doctor"
        onLogout={onLogout}
      />
      <section className="wrap section dashboard-grid doctor-grid">
        <Panel title="Today's queue">
          {data.appointments.length ? (
            ...
            <Empty text="No appointments in your queue." />
          )}
        </Panel>
        ...
        <Panel title="Assessment workspace">
          {selected ? (
            <form className="form" onSubmit={save}>
              <p className="muted">Consultation {selected._id}</p>
              <label>
                Symptoms
                <textarea
                  name="symptoms"
                  defaultValue={selected.symptoms || ""}
                />
              </label>
              <label>
                Assessment
                <textarea
                  name="assessment"
                  required
                  defaultValue={selected.assessment || ""}
                />
              </label>
              <label>
                Advice
                <textarea name="advice" defaultValue={selected.advice || ""} />
              </label>
              <label>
                Prescription
                <textarea
              ...
              <label>
                ...
                <select
                  name="outcome"
                  defaultValue={selected.outcome || "not_urgent"}
                >
            ...
            <Empty text="Select a consultation to edit assessment." />
          )}
        </Panel>
      </section>
      {call && (
